...
import mceq_underground_helpers_oneregion_thresholdintegration as mh
...
def dNmu_dmu(d,month, ptype, cs_p, cs_k, e0,threshold): # month = str
    '''
    ...
    for i in range(len(x_mod)):
        ...
        dNmudmu[i] = 1e4*mh.mult_dist(x_mod[i],
                                ...
                                ) / mh.rates(x_mod[i], angle, month, ptype, cs_p, cs_k, e0,threshold)
    return dNmudmu
...
def R_normalized_threshold(m,R_mod,d,ptype,e0,threshold):
    '''
        Normalization of R to april as default atmsophere and Sibyll2.3c
    ...
    '''
    # default parameters
    dNu_dmu_apr = dNmu_dmu(d,month="apr", ptype=ptype, cs_p=1.0, cs_k=1.0, e0=e0,threshold=threshold) #default cs
    R_def_apr = R(m,dNu_dmu_apr)
...
@click.command()
@click.option('--threshold','-t', help='1 or 500 for 1 or 500 GeV')
                ...
                for ptype in ptype_values:
                    for season in season_values:
                        for e0 in e0_values:
                            ...
                            dNmu_dmu_mod = dNmu_dmu(d,season, ptype , cs_p, cs_k, e0,thres)
                            R_mod = R(m,dNmu_dmu_mod)
                            R_norm = R_normalized_threshold(m,R_mod,d,ptype,e0,thres)    
                                                            
                            # Store the result in the dictionary
                            results[(str(d), str(cs_p), str(cs_k), str(ptype), season, str(e0))] = R_norm

    with open("/hetghome/khymon/cs-files/R_value_const_pi-air_k-air_sibyll23c_smooth_oneregion_intthreshold" + str(threshold) + "gev.pkl", "wb") as f:
