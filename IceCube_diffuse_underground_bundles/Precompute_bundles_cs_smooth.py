...
# Import the class
from cs_modifier_with_smooth_transition import ModIntCrossSections


@click.command()
@click.option('--scale_factor_p1','-a', help='cs modification for region 1 pion, 1 = untuned for const, 0 = untuned for exp') 
@click.option('--scale_factor_p2','-b', help='cs modification for region 1 pion, 1 = untuned for const, 0 = untuned for exp') 
...
def main(scale_factor_p1, scale_factor_p2, scale_factor_k1, scale_factor_k2, e0, e1,increase,interactionmodel,nucleus):
    ...
    pickle.dump(
        ...
        open(f"/hetghome/khymon/cs-files/smooth-transition/ground_muspec_prim_energies_season_cstune{nucleus}"
            f"_pi{float(scale_factor_p1):.2f}_{float(scale_factor_p2):.1f}"
            f"_k{float(scale_factor_k1):.1f}_{float(scale_factor_k2):.1f}"
